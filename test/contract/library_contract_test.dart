...
void main() {
  group('library contract', skip: contractSkipReason, () {
    ...
    test(
      'GET /library/folders decodes folder tree into LibraryFolder',
      () async {
    ...
    test(
      'GET /library/stats decodes library telemetry into LibraryStats',
      () async {
    ...
    test(
      'folder lifecycle: create, rename, list and delete round-trip',
      () async {
        final folderName =
            'contract-folder-${DateTime.now().millisecondsSinceEpoch}';
        ...
        final created = folders.firstWhere(
          ...
          orElse: () => throw StateError(
            'Created folder $folderName not found in listFolders',
          ),
        );
        ...
        expect(
          updatedFolders.any(
            (f) => f.id == created.id && f.name == renamedName,
          ),
          isTrue,
        );
    ...
    test(
      'GET /library/files/{id}/plates decodes plates from seeded file',
      () async {
        final files = await library.listFiles();
