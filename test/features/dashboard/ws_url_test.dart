...
void main() {
  group('wsUrlFor', () {
    test('https → wss with /api/v1/ws path', () {
      expect(
        wsUrlFor('https://bambu.morganmlg.com').toString(),
    ...
    test('http → ws (plain local server)', () {
      expect(
        wsUrlFor('http://192.168.1.10:8000').toString(),
    ...
    test('preserves a non-default port', () {
      expect(
        wsUrlFor('https://host:8443').toString(),
    ...
    test('preserves a base path prefix (reverse-proxy subpath)', () {
      expect(
        wsUrlFor('https://host/proxy').toString(),
