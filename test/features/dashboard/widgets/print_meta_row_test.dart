...
void main() {
  ...
  testWidgets('too wide for one line: two columns that line up', (
    ...
    expect(
      at[2].dx,
      at[0].dx,
      reason: 'the second line starts a column, not a run',
    );
    expect(
      at[1].dx - at[0].dx,
      columnWidth + _gap,
      reason:
          'the ETA sits in the second column — a Wrap would butt it '
  ...
  testWidgets('too narrow even for two columns: one column', (tester) async {
    ...
    expect(
      {for (final corner in at) corner.dx},
      {0.0},
  ...
  testWidgets('measures the label the way it is painted, spacing and all', (
    ...
    final onOneLine = tester.getTopLeft(last).dx + tester.getSize(last).width;

    await _pump(tester, width: onOneLine - 1, ambient: ambient);

    expect(tester.takeException(), isNull);
    expect(
      _corners(tester)[2].dy,
      greaterThan(_corners(tester)[0].dy),
