...
void main() {
  ...
  testWidgets('the filled action fits the bar at a large text size',
    ...
    expect(find.widgetWithText(FilledButton, formL10n.queueCreateSubmit),
        findsOneWidget);
  });
  ...
  testWidgets('with no printer chosen nothing reaches the server',
      (tester) async {
    await tester
        .pumpWidget(queueFormScreen(archiveDraft(printerId: null)));
    await tester.pumpAndSettle();
  ...
  group('auto-print G-code injection', () {
    ...
    testWidgets('the ticked option rides with the POST', (tester) async {
      ...
      expect(storedPrintOptions().gcodeInjection, isTrue,
    ...
    testWidgets('a remembered option does not reach a server with no snippets',
        (tester) async {
    ...
    testWidgets('a target model with no snippet says nothing will be injected',
        (tester) async {
      ...
      expect(find.text(formL10n.queueEditGcodeInjectionNoSnippet('X2D')),
          findsNothing,
  ...
  group('tri-state calibrations', () {
    ...
    testWidgets('an older server does not offer the Auto position',
      ...
      expect(find.text(formL10n.commonAuto), findsNothing,
    ...
    testWidgets('a 1.2.5 server offers Auto / On / Off', (tester) async {
      await tester
          .pumpWidget(queueFormScreen(archiveDraft(), triState: true));
      await tester.pumpAndSettle();
    ...
    testWidgets('a chosen Auto is remembered for the next print',
      ...
      expect(storedPrintOptions().flowCali, CalibrationOption.on,
