...
void main() {
  ...
  String read(String path) {
    ...
    if (!file.existsSync()) {
      throw StateError(
        '$path is gone, and the mirror it holds is now '
  ...
  test('the claim is written under the key the service reads', () async {
    ...
    expect(
      service,
      contains('"flutter.${prefs.getKeys().single}"'),
      reason:
          'the service would read an empty claim and wake a second '
  ...
  test('the entry point the host names is the one that exists', () {
    ...
    expect(
      entryLibrary,
      contains('void wearRelayMain('),
      reason:
          'it must stay in the app entry library — in one of its own it '
  ...
  test('the ack the service writes is the one the watch decodes', () {
    ...
    for (final field in ack.keys) {
      expect(
        ...
        contains('"$field"'),
        reason:
            'the watch would not find `$field` in the ack and would '
