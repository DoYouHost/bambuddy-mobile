...
void main() {
  ...
  group('what a notification must never leave behind', () {
    ...
    test('a refused alert reports the exception class, never its message',
        () async {
    ...
    test('the ongoing record carries numbers, not the job it describes',
        () async {
  ...
  group('why no alert arrived — the half that leaves no other trace', () {
    ...
    test('"you turned it off" and "everything is off" stay different answers',
        () async {
      // Two different controls, two different things to tell the user to go
      // and change.
      NotifProbe.suppressed(NotifSkip.typeOff, event: NotifEvent.printFinished);
    ...
    test('a fleet-wide decision names no printer rather than guessing one',
        () async {
    ...
    test('priming is not a suppression, because nothing was decided yet',
        () async {
    ...
    test('a print that ended in a state nothing acts on is flagged as unhandled',
        () async {
      ...
      expect(rows.last.containsKey('lvl'), isFalse, reason: 'info is implied');
    });
  });
  ...
  group('the settings snapshot that explains the rest of the session', () {
    ...
    test('a platform read that throws costs the field, not the snapshot',
        () async {
      // A recorder must not be able to cause the failure it exists to
      // describe.
      await NotifProbe.openSession(
        const NotificationPrefs(enabled: {NotifEvent.printFinished}),
        permission: () async => throw StateError('no platform in a unit test'),
