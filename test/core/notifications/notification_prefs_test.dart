...
void main() {
  ...
  group('an event the writing build did not know', () {
    test("takes this build's default, not \"off\"", () {
      ...
      expect(
        decoded.isOn(NotifEvent.printStarted),
        isTrue,
        reason: "user's choice",
      );
      expect(
        decoded.isOn(NotifEvent.printFinished),
        isFalse,
    ...
    test(
      'migration: a payload without the manifest leaves choices untouched',
      () {
  ...
  test('corrupt/empty string → default prefs', () {
    expect(
      NotificationPrefs.decode(null).enabled,
      NotificationPrefs.defaults.enabled,
    );
    expect(
      NotificationPrefs.decode('not json').enabled,
      NotificationPrefs.defaults.enabled,
    );
  });
  ...
  test(
    'master alertsEnabled=false mutes every event without losing choices',
    () {
      const prefs = NotificationPrefs(
