...
/// `backend/app/schemas/auth.py:24` — only `id` and `name` are sent, the
/// group's own permission list is not).
class UserGroup {
...
/// (`backend/app/schemas/auth.py:80`). Comes from two places that send the
/// identical shape: the `user` object inside `POST /auth/login`'s answer, and
/// `GET /auth/me` when a session is restored.
class CurrentUser {
  const CurrentUser({
    required this.id,
  ...
  factory CurrentUser.fromJson(Map<String, dynamic> json) => CurrentUser(
        ...
        // Key presence, not "has entries": the server declares
        // `permissions: list[str] = []` and computes it as the union over the
        // user's groups (`backend/app/models/user.py:99`), so an empty list is
  ...
  /// (`backend/app/models/user.py:110`), which is why [can] short-circuits on
  /// it rather than looking for the permission in [permissions].
  final bool isAdmin;
  ...
  /// Whether this user may do [permission] — mirroring
  ...
  /// (`backend/app/api/routes/auth.py:93`). Neither is a substitute for the
  /// gate: the routes refuse a key every administrative permission outright,
  /// whatever it was told. Anything gating on users/groups/api-keys must go
  /// through `identifiedPermissionProvider`, which accounts for that.
...
abstract final class Permissions {
  /// `backend/app/core/permissions.py:170`
  static const usersRead = 'users:read';

  /// `backend/app/core/permissions.py:176`
