/// The two roles the server accepts (`users.py::create_user`, `:289` — anything
/// else is a 400). `is_admin` is computed from this *or* group membership
/// (`backend/app/models/user.py::get_permissions`), and bambuddy's own UI only
/// ever sends `user`, granting admin by putting the account in the
/// Administrators group. The app does the same, so [admin] is here to name the
/// value the server may send back, not one this app writes.
abstract final class UserRoles {
  static const admin = 'admin';
  static const user = 'user';
}

/// Body for `POST /users/` (`backend/app/schemas/auth.py::UserCreate`).
///
/// [password] is omitted when the server has advanced authentication on — it
/// generates one and mails it (`users.py::create_user`), and refuses the
/// request without an [email]. With advanced authentication off the reverse
/// holds: the password is required and the e-mail optional.
...
/// `is not None` (`users.py::update_user`), so a caller must send only what it
/// means to change. That is also why an e-mail is cleared by sending an empty
...
/// (`backend/app/schemas/auth.py::_validate_password_complexity`). Mirrored so
/// the form can say which rule is unmet instead of showing the 422 that would
/// come back.
...
/// generated password is best-effort server-side (`users.py::create_user` logs
/// the failure and creates the account anyway), so an admin creating an account
